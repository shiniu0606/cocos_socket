...
private:
	...
	void MD5Init();
	//置位函数
	void MD5_memset(unsigned char * output, int value, size_t len);
	//拷贝函数
	...
	//解码函数
	void Decode(unsigned int *output, unsigned char * input, size_t len);
};
...
//更新函数
void CMD5::MD5Update (unsigned char * input, size_t inputLen)
{
...
//编码函数
void CMD5::Encode(unsigned char * output, unsigned int * input,size_t len)
{
	unsigned int i, j;
...
//解码函数
void CMD5::Decode(unsigned int *output, unsigned char *input, size_t len)
{
	unsigned int i,j;
	for (i=0,j=0;j<len;i++,j+=4)
	{
...
//拷贝函数
void CMD5::MD5_memcpy(unsigned char * output, unsigned char * input,size_t len)
{
	for (unsigned int i=0;i<len;i++) output[i]=input[i];
}

//置位函数
void CMD5::MD5_memset (unsigned char * output, int value, size_t len)
{
