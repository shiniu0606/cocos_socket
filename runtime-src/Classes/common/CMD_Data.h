		...
		if (argc == 3)
		{
			...
			do
			{
				if (!EMPTY_CHAR(szData))
				{
					...
					char* szSrc = new char[wDstLen];
					memset(szSrc, 0, wDstLen);
					memcpy(szSrc, szData, wDstLen);
					cobj->PushByteData((BYTE*)szSrc, wDstLen);
					CC_SAFE_DELETE(szSrc);
				}
				else
				{
					cobj->PushByteData(NULL, wDstLen);
				}
...
static int toLua_Cmd_Data_pushMD5(lua_State* tolua_S)
{
	CCmd_Data* cobj = (CCmd_Data*)tolua_tousertype(tolua_S, 1, nullptr);
	if (cobj)
	{
		...
		if (argc == 3)
		{
			...
			do
			{
				if (!EMPTY_CHAR(szData))
				{
					WORD wSrcLen = strlen(szData);
					char* szSrc = new char[wDstLen];
					CMD5Encrypt::EncryptData(szData, szSrc);
					cobj->PushByteData((BYTE*)szSrc, wDstLen);
					CC_SAFE_DELETE(szSrc);
				}
				else
	...
	if (cobj)
	{
		...
		if (argc == 1)
		{
			...
			if (wIndex<cobj->GetBufferLenght())
			{
				...
				char* pszSrcData = new char[wMaxLen+1];

				memset(pszSrcData, 0, wMaxLen+1);

				memcpy(pszSrcData, cobj->m_pBuffer + wIndex, wMaxLen);

				lua_pushstring(tolua_S, pszSrcData);
				CC_SAFE_DELETE(pszSrcData);
				cobj->SetCurrentIndex(wIndex + wMaxLen);
				return 1;
			}
			else {
		...
		if (argc == 2)
		{
			...
			if (wMaxLen>0 && wIndex + wMaxLen <= cobj->GetBufferLenght())
			{
				char*	pszDstData = new char[wMaxLen * 2 + 1];
				char*	pszSrcData = new char[wMaxLen + 1];
				...
				std::string strVal = a_u8(pszSrcData);
				lua_pushlstring(tolua_S, strVal.c_str(), strVal.size());
				CC_SAFE_DELETE(pszSrcData);
				cobj->SetCurrentIndex(wIndex + wMaxLen);
				return 1;
			}
...
static int toLua_Cmd_Data_readCHAR(lua_State* tolua_S)
{
	//CCLOG("toLua_Cmd_Data_readCHAR");
	CCmd_Data* cobj = (CCmd_Data*)tolua_tousertype(tolua_S, 1, nullptr);
	if (cobj)
	{
		int argc = lua_gettop(tolua_S);
		...
		if (argc == 2)
		{
			...
			if (wMaxLen > 0 && wIndex + wMaxLen <= cobj->GetBufferLenght())
			{
				wMaxLen = cobj->GetBufferLenght() - wIndex;
			}
			char*	pszSrcData = new char[wMaxLen + 1];
...
static int register_all_cmd_data()
{
	...
	tolua_function(tolua_S, "pushmd5", toLua_Cmd_Data_pushMD5);
	...
	tolua_function(tolua_S, "readtchar", toLua_Cmd_Data_readCHAR);
