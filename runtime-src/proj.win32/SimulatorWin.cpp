...
int SimulatorWin::run()
{
	...
	if (args.size() == 1)
	{
		string filePath = StringUtil::ExtractDirName(args.at(0));
		filePath = StringUtil::ExtractDirName(filePath);
		filePath = StringUtil::ExtractDirName(filePath);
...
void SimulatorWin::parseCocosProjectConfig(ProjectConfig &config)
{
	...
	if (args.size() == 1)
	{
		...
		args.push_back(filePath);
		filePath += "/";
		FileUtils::getInstance()->addSearchPath(filePath.c_str());
		FileUtils::getInstance()->setDefaultResourceRootPath(filePath.c_str());
	}
	else
	{
